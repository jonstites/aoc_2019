...
fn main() -> Result<(), ExitFailure> {
    ...
    let min_sum = get_min_sum(wire1, wire2)?;
    println!("{}", min_sum);
    Ok(())
}

fn get_min_sum(wire1: &str, wire2: &str) -> Result<i32, ExitFailure> {
...
fn find_intersections(wire1: Vec<(i32, i32)>, wire2: Vec<(i32, i32)>) -> Vec<i32> {
	...
	for _x in 0..direction.0 {
	    ...
	    num_steps += 1;	    
	    let steps = points.entry(current).or_insert(num_steps);
	    *steps = num_steps.min(*steps);
	}
	...
	for _x in 0..direction.0 {
	    current = (current.0 + 1, current.1);
	    num_steps += 1;	    	    
	    if let Some(&steps) = points.get(&current) {
		intersections.push(num_steps + steps);
	    }
	}
	for _x in direction.0..0 {
	    current = (current.0 - 1, current.1);
...
fn find_intersections_ok() {
    ...
    assert_eq!(find_intersections(w1, w2), vec!(30, 40));
}
